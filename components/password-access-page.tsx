...
export function PasswordAccessPage({
  eyebrow,
  title,
  description,
  submitLabel,
  ...
  useEffect(() => {
    ...
    function markInvalid(error?: unknown) {
      ...
      console.warn("[password-access] Password link could not establish a session.", error);
    ...
    async function resolveSession() {
      try {
        ...
        if (authParams.code) {
          console.info("[password-access] Exchanging Supabase auth code for password setup.");
          ...
          console.info("[password-access] Verifying Supabase OTP token for password setup.");
          ...
          console.info("[password-access] Restoring Supabase session from password setup tokens.");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (sessionError) {
        console.error("[password-access] Failed to load session before password update.", sessionError);
        setErrorMessage(getPasswordSetupErrorMessage(sessionError));
        return;
      }

      if (!sessionData.session && sessionSnapshot) {
        console.warn("[password-access] Session missing before password update. Restoring from setup snapshot.");
        ...
        if (restoreError) {
          console.error("[password-access] Failed to restore session before password update.", restoreError);
      ...
      if (error) {
        console.error("[password-access] Password update failed.", error);
      ...
      console.error("[password-access] Password update request crashed.", error);
      setErrorMessage(getPasswordSetupErrorMessage(error));
    } finally {
      setIsPending(false);
    }
        ...
        <div className="w-full max-w-[480px]">
          <section className="surface-primary w-full px-8 py-9 shadow-[0_18px_55px_rgba(17,24,39,0.08)] sm:px-10 sm:py-11">
            ...
            <div className="mt-7">
              ...
              <h1 className="type-page-title text-[32px]">{title}</h1>
              <p className="type-body-text mt-3 text-[15px]">{description}</p>
            </div>
            ...
            <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
              ...
              {setupState === "checking" ? (
                <div className="rounded-[12px] border border-[#DBEAFE] bg-[#EFF6FF] px-4 py-3 text-[14px] text-[#1D4ED8]">
                  Verifying your reset link and preparing password setup.
                </div>
              ) : null}
            ...
            <div className="surface-secondary mt-6 px-4 py-4">
              ...
              <p className="type-body-text mt-2 text-[14px]">
                Accounts are created by invitation only. If your link has expired, request a fresh reset email from the
                sign-in page.
              </p>
            </div>
          </section>
